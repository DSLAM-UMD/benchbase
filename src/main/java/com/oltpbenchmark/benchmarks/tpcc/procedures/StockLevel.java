...
public class StockLevel extends TPCCProcedure {
    ...
    private int getOrderId(Connection conn, PartitionedWId w_id, int d_id) throws SQLException {
        SQLStmt stockGetDistOrderIdSQL = new SQLStmt(
                """
                            SELECT D_NEXT_O_ID
                                FROM  %s_%s
                                WHERE D_W_ID = ?
        ...
        try (PreparedStatement stockGetDistOrderId = this.getPreparedStatement(conn, stockGetDistOrderIdSQL)) {
            stockGetDistOrderId.setInt(1, w_id.id);
    ...
    private int getStockCount(Connection conn, PartitionedWId w_id, int threshold, int d_id, int o_id)
            throws SQLException {
        SQLStmt stockGetCountStockSQL = new SQLStmt(
                """
